...
system_message = "You are an agent that can read bills and return a list of items with their costs." \
...
def process(uploaded_file):
    ...
    image_url = {
        "url": f"data:{uploaded_file.type};base64,{base64_str}",
        "format": uploaded_file.type
    }

    try:
        resp = rt.call_sync(spliter_agent, user_input=rt.llm.MessageHistory([rt.llm.UserMessage("Read this bill and tell me the items and their costs")]), image_url=image_url)
        struct_resp = resp.structured   # type: ignore 
        assert isinstance(struct_resp, SpliterOutput), "Response is not of type SpliterOutput"
        return struct_resp
    except AssertionError as e:
        return SpliterOutput(items=[
